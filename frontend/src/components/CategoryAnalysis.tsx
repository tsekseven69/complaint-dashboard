...
import {
  CategoryAnalysisResult, CategoryDetail, CategoryGroup, GroupedCategories,
  OrgDetail, FilterParams,
  fetchCategoryAnalysis, fetchGroupedCategories, fetchOrgDetail,
} from '../api/client'
import { ChevronDown, ChevronRight, Building2, X } from 'lucide-react'

export default function CategoryAnalysis({ fp }: { fp: FilterParams }) {
  const [data, setData] = useState<CategoryAnalysisResult | null>(null)
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([
      fetchCategoryAnalysis(fp).catch(() => null),
      fetchGroupedCategories(fp).catch(() => null),
    ]).then(([d, g]) => {
      setData(d)
      setGrouped(g)
  ...
  const showOrgDetail = async (orgName: string) => {
    setOrgLoading(true)
    try {
      const detail = await fetchOrgDetail(orgName, fp)
      setOrgDetail(detail)
    } catch {
      // ignore
    } finally {
    ...
    <div>
      ...
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        ...
        <div className="stat-card resolved">
          <div className="label">Ангилагдсан</div>
          <div className="value">{data.categorized_count}</div>
        </div>
        <div className="stat-card rate">
          <div className="label">Ангилалын тоо</div>
          <div className="value">{data.categories.length}</div>
        </div>
        {grouped && (
          <div className="stat-card" style={{ borderLeftColor: '#00acc1' }}>
            <div className="label">Бүлгийн тоо</div>
            <div className="value">{grouped.groups.length}</div>
          </div>
        )}
      </div>
      ...
      <div className="tabs" style={{ marginBottom: 20 }}>
        <button className={`tab ${viewMode === 'grouped' ? 'active' : ''}`} onClick={() => setViewMode('grouped')}>
          Бүлгээр
        </button>
        <button className={`tab ${viewMode === 'detailed' ? 'active' : ''}`} onClick={() => setViewMode('detailed')}>
          Дэлгэрэнгүй ангилал
        </button>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setOrgDetail(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 800 }}>
            ...
            <h3 style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Building2 size={20} /> {orgDetail.org}
            </h3>
            <div className="stats-grid" style={{ marginBottom: 16 }}>
              <div className="stat-card">
                <div className="label">Нийт</div>
                <div className="value">{orgDetail.total}</div>
              </div>
              <div className="stat-card resolved">
                <div className="label">Шийдвэрлэсэн</div>
                <div className="value">{orgDetail.resolved}</div>
              </div>
              <div className="stat-card rate">
                <div className="label">Хувь</div>
                <div className="value">{orgDetail.resolution_rate}%</div>
              </div>
            </div>
                  ...
                  {orgDetail.content_categories.map((c) => (
                    <div key={c.category} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'var(--bg)', borderRadius: 8, fontSize: 13 }}>
                      ...
                      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                        ...
                        <span style={{ fontWeight: 600, color: c.resolution_rate >= 50 ? 'var(--success)' : c.resolution_rate > 0 ? 'var(--warning)' : 'var(--danger)' }}>
                          {c.resolution_rate}%
                        </span>
                      </div>
                    </div>
      ...
      {orgLoading && <div className="loading" style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(255,255,255,0.5)' }}><div className="spinner" /></div>}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {grouped.groups.map((group) => (
            <GroupCard
              key={group.group}
              group={group}
              isExpanded={expandedGroups.has(group.group)}
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: rateColor }}>
            {group.resolution_rate}% шийдвэрлэсэн
          </span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {cat.statuses.map((s) => (
                  <div key={s.status} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                    <span>{s.status || 'Тодорхойгүй'}</span>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {cat.orgs.length > 0 ? cat.orgs.map((o) => (
                  <div
                    key={o.org}
                    style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, cursor: 'pointer', padding: '2px 0' }}
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {cat.sample_complaints.map((c) => (
                <div key={c.complaint_id} style={{ background: 'var(--bg)', borderRadius: 8, padding: '10px 14px', fontSize: 13, lineHeight: 1.5 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontWeight: 600, color: 'var(--primary)' }}>#{c.complaint_number}</span>
                    <span className={`status-badge ${c.resolution_status?.includes('Шийдвэрлэсэн') ? 'resolved' : 'pending'}`}>
                      {c.resolution_status || 'Хүлээгдэж буй'}
                    </span>
                  </div>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {cat.sample_responses.map((r, i) => (
                  <div key={i} style={{ background: '#e6f4ea', borderRadius: 8, padding: '10px 14px', fontSize: 13, lineHeight: 1.5, borderLeft: '3px solid var(--success)' }}>
