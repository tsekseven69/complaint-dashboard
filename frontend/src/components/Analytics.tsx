...
const INSIGHT_STYLES: Record<string, { bg: string; border: string; color: string; icon: string }> = {
  info: { bg: '#e8f0fe', border: '#1a73e8', color: '#174ea6', icon: 'info' },
  warning: { bg: '#fef7e0', border: '#f9ab00', color: '#b06000', icon: 'warn' },
    ...
    <div>
      ...
      <div className="tabs" style={{ marginBottom: 20 }}>
        ...
        <button className={`tab ${subTab === 'content' ? 'active' : ''}`} onClick={() => setSubTab('content')}>
          <Tag size={14} style={{ verticalAlign: 'middle', marginRight: 5 }} />
          Агуулгын ангилал
        </button>
      ...
      {subTab === 'trends' && trends && <TrendsView data={trends} />}
      {subTab === 'resolution' && resolution && <ResolutionView data={resolution} />}
      {subTab === 'content' && content && <ContentView data={content} />}
      {subTab === 'edges' && content && <EdgesView data={content} />}
    </div>
  )
}
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 12, flexWrap: 'wrap' }}>
          {data.daily.map(d => (
            <div key={d.date} style={{
              padding: '8px 14px',
              borderRadius: 8,
              background: d.change > 0 ? '#fce8e6' : d.change < 0 ? '#e6f4ea' : '#f8f9fa',
              fontSize: 13,
            }}>
              <strong>{d.date.slice(5)}</strong>: {d.count} гомдол
              {d.change !== 0 && (
                <span style={{ marginLeft: 6, color: d.change > 0 ? '#ea4335' : '#34a853' }}>
                ...
                <tr key={i}>
                  ...
                  <td>
                    <span style={{
                      ...
                      {ct.direction === 'өсөлт' ? <TrendingUp size={13} /> :
                       ct.direction === 'бууралт' ? <TrendingDown size={13} /> :
                       <Minus size={13} />}
                      {ct.direction}
                    </span>
                  </td>
...
function ResolutionView({ data }: { data: ResolutionAnalysis }) {
  const catData = data.by_category.slice(0, 12).map(c => ({
    name: c.category.length > 35 ? c.category.slice(0, 35) + '...' : c.category,
    resolved: c.resolved,
    pending: c.pending,
    rate: c.rate,
  }))

  const orgData = data.top_responding_orgs.map(o => ({
    name: o.org.length > 45 ? o.org.slice(0, 45) + '...' : o.org,
    count: o.count,
  }))

  const coverageData = [
    { name: 'Хариу өгсөн', value: data.response_coverage.with_response },
    { name: 'Хариу өгөөгүй', value: data.response_coverage.without_response },
  ]
      ...
      <div className="chart-card" style={{ gridColumn: '1 / -1' }}>
        ...
        <ResponsiveContainer width="100%" height={400}>
          <BarChart data={catData} layout="vertical" margin={{ left: 20 }}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="resolved" stackId="a" fill="#34a853" name="Шийдвэрлэсэн" radius={[0, 0, 0, 0]} />
            <Bar dataKey="pending" stackId="a" fill="#f9ab00" name="Хүлээгдэж буй" radius={[0, 4, 4, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>

      {/* Response coverage pie */}
        ...
        <div className="table-wrapper">
          <table>
            <thead>
              <tr>
                <th>Дүүрэг</th>
                <th style={{ textAlign: 'center' }}>Нийт</th>
                <th style={{ textAlign: 'center' }}>Шийдвэрлэсэн</th>
                <th style={{ textAlign: 'center' }}>Хувь</th>
                <th>Прогресс</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <td style={{ minWidth: 120 }}>
                    <div style={{
                      ...
                      <div style={{
                        width: `${d.rate}%`,
                        height: '100%',
                        background: d.rate >= 30 ? '#34a853' : d.rate >= 10 ? '#f9ab00' : '#ea4335',
                        borderRadius: 6,
...
// ── Content Classification View ──────────────────────────────────────────

function ContentView({ data }: { data: ContentClassification }) {
  const themeData = data.themes.map(t => ({
    name: t.theme,
    count: t.count,
  }))
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 14 }}>
          {data.themes.map((theme, ti) => (
            <div key={ti} style={{
              ...
              borderRadius: 10,
              padding: 14,
              borderTop: `3px solid ${COLORS[ti % COLORS.length]}`,
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <strong>{theme.theme}</strong>
                <span style={{
                  background: COLORS[ti % COLORS.length] + '20',
              ...
              {theme.examples.map((ex, ei) => (
                <div key={ei} style={{
                  ...
                  padding: '6px 0',
                  borderTop: ei > 0 ? '1px solid #f0f0f0' : 'none',
                  lineHeight: 1.4,
                }}>
                  <span style={{ fontWeight: 600, color: 'var(--text)' }}>#{ex.complaint_number}</span>
                  {' '}{ex.description}
                </div>
              ))}
            </div>
...
function EdgesView({ data }: { data: ContentClassification }) {
  const FLAG_COLORS: Record<string, string> = {
    ...
    'Давтагдсан': '#f9ab00',
    'Дэлгэрэнгүй': '#1a73e8',
    'Нууцлал хүссэн': '#8e24aa',
    'Олон байгууллага': '#00acc1',
    'Хүүхэдтэй холбоотой': '#e8710a',
  }
              ...
              {data.edge_complaints.map((ec, i) => (
                <tr key={i}>
                  ...
                  <td style={{ whiteSpace: 'nowrap', fontSize: 12 }}>
                    {ec.district?.replace('Улаанбаатар, ', '') || '—'}
                  </td>
                    ...
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                      {ec.flags.map((f, fi) => (
                        <span key={fi} style={{
                          ...
                          fontSize: 11,
                          fontWeight: 500,
                          background: (FLAG_COLORS[f] || '#5f6368') + '18',
                          color: FLAG_COLORS[f] || '#5f6368',
                          border: `1px solid ${(FLAG_COLORS[f] || '#5f6368')}40`,
                        }}>
                          {f}
                        </span>
                      ))}
                  ...
                  <td>
                    <span className={`status-badge ${ec.resolution_status ? 'resolved' : 'pending'}`}>
                      {ec.resolution_status ? 'Шийдвэрлэсэн' : 'Хүлээгдэж буй'}
                    </span>
                  </td>
                </tr>
              ))}
